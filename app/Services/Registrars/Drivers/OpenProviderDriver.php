...
class OpenProviderDriver extends AbstractRegistrarDriver
{
    protected function getDefaultEndpoint(): string
    {
        return $this->sandbox
            ? 'https://api.pte.openprovider.eu/v1beta/'
    ...
    public function checkAvailability(string $domain): array
    {
        ...
        $name = $parts[0];
        $extension = $parts[1] ?? 'com';

        $payload = [
            'username' => $this->config['username'] ?? 'demo',
            'password' => $this->config['password'] ?? 'demo',
            'domains' => [
        ...
        try {
            ...
            $response = [
                ...
                'data' => [
                    [
                        'domain' => $domain,
                        'status' => $available ? 'free' : 'active',
                    ]
                ],
            ];

            $execTime = (int) ((microtime(true) - $startTime) * 1000);
            $this->logApiCall('checkAvailability', $this->endpoint . 'domains/check', $payload, $response, 200, $execTime);
            ...
            $this->logApiCall('checkAvailability', $this->endpoint . 'domains/check', $payload, [], 500, 0, $e->getMessage());
            return [
                'domain' => $domain,
                'available' => false,
                'status' => 'ERROR',
    ...
    {
        return [$keyword . 'app.io', $keyword . 'store.co.ke', $keyword . 'tech.com'];
    }
    ...
    public function registerDomain(array $params): array
    {
        ...
        $this->logApiCall('registerDomain', $this->endpoint . 'domains', $params, ['id' => rand(10000, 99999), 'status' => 'active'], 200, 150);
    ...
    {
        return ['success' => true, 'domain' => $params['domain'] ?? '', 'status' => 'PENDING'];
    }
