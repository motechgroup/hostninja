...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'full_setup') {
            ...
            $outputLog[] = "<strong>Clear Cache Result:</strong>\nAll caches (config, route, view, app cache) cleared successfully.";
        } elseif ($action === 'optimize') {
            Artisan::call('config:cache');
            Artisan::call('route:cache');
            Artisan::call('view:cache');
            $outputLog[] = "<strong>Optimize Caches Result:</strong>\nConfiguration, routes, and views compiled and cached successfully.";
        } elseif ($action === 'custom' && !empty($customCmd)) {
            // Strip leading php artisan if user included it
...
<head>
    ...
    <title>HostNinja Web Deployment & Artisan Console</title>
...
<body>
    <div class="container">
        ...
        <?php if (!empty($keyGeneratedNotice)): ?>
            <div class="notice">
                ✨ <strong>App Key Configured:</strong> <?php echo htmlspecialchars($keyGeneratedNotice); ?>
            </div>
        <?php endif; ?>
